...
export function Progress({
  ...
  return (
    <ProgressPrimitive.Root
      {...props}
      className={cn("w-full grid grid-cols-2 gap-y-2", className)}
    >
      <ProgressPrimitive.Label className="text-nowrap">
        {children}
      ...
      <ProgressPrimitive.Track className="col-span-full h-3 overflow-hidden rounded bg-slate-300">
        <ProgressPrimitive.Indicator className="block bg-slate-500 transition-all duration-500" />
