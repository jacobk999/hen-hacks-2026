...
const genAI = new GoogleGenerativeAI("AIzaSyDkBWYC___tz0JPAFAbOyebTVaorETSz50");
const model = genAI.getGenerativeModel({
  model: "gemini-2.5-flash", // Updated to the stable flash model name
  generationConfig: {
    responseMimeType: "application/json",
  },
});

export async function generateMultipleDynamicEvents({
  day,
  stats,
  lines,
  count = 5,
...
> {
  ...
  const prompt = `
    ...
    4. Customer Trust (Aggressive): Low Station Quality makes Satisfaction gains 0.3x slower and losses 1.5x faster.
    ...
    - Star Ratings: Small increments (-0.4 to +0.3). Use (+,-) [0.5 - 1.2] only for massive Day 50+ shifts.
    - Employees: Small shifts (1-5). Disasters can be 10-20.
    - Money: Low ($1k - $15k), Moderate ($20k - $40k), High ($45k - $100k).
    - DailyProfit/Expenses: Modify these for permanent economic shifts.

    TASK: Generate exactly ${count} unique and distinct events.
    Ensure each event focuses on a different 'Critical Mechanic'. For example: one event targeting Understaffing, one targeting Customer Trust, and one targeting Star Ratings.
  ...
  if (!rawEvents || !Array.isArray(rawEvents)) {
    console.error("AI Response missing events array:", data);
    return []; // Return empty array to prevent .map crash
  }

  return rawEvents.map((jsonEvent: any) => ({
    ...jsonEvent,
    id: jsonEvent.id || `ai-event-${Date.now()}-${Math.random()}`, // Ensure ID exists
    repeatable: false,
    weight: 10000,
    criteria: () => true,
